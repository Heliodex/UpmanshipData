...
def write(fileRaw: str, outDir: str):
    ...
    out = open(f"{outDir}/LongestHiatusSeconds.md", "w+", encoding="utf-8")
    longestHiatus = {}
    startTime = time()
    ...
    for j in longestHiatus:
        ...
        for i in range(len(table)):
            if table["author"][i] == j:
                ...
                elif longestHiatus[j] < timestamp - prevComment:
                    longestHiatus.update(
                        {j: timestamp - prevComment})
    ...
    out.write("**User**|**Longest Hiatus (seconds)**\n:-|-:\n")
